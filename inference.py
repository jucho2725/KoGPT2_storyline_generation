...
class GPT2:
    ...
    def generation_fromutil(
        ...
        for _ in range(repeat):
            output = self.kogpt2model.generate(
                input_ids=input_ids,
                eos_token_id=1,
                pad_token_id=3,
                do_sample=True,
...
if __name__ == "__main__":
    ...
    ex = [
        "반복되는 일상을 못 견디던 엘리엇은 세탁소에서 만난 제인과 사랑에",
        "헬렌과 함께 만나게 된 일행들은 지하실로 내려가지만, 그곳에는",
        "지구에서 탈출한 키모아와 그의 친구들은 수수께끼의 행성 N95로",
    ]
    ...
    for i in range(3):
        ...
        res_l = model.generation_fromutil(
            genres=gen[i],
            input_sentence=ex[i],
            temperature=0.9,
            top_p=0.8,
